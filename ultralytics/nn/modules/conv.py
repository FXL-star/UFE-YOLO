...
class Focus(nn.Module):
    ...
    """
    工作原理
        切片操作：将输入特征图按像素间隔分成 4 个子区域（类似棋盘格采样）：
        x[..., ::2, ::2]：左上子区域（偶数行、偶数列）
        x[..., 1::2, ::2]：右上子区域（奇数行、偶数列）
        x[..., ::2, 1::2]：左下子区域（偶数行、奇数列）
        x[..., 1::2, 1::2]：右下子区域（奇数行、奇数列）
        通道拼接：将这 4 个子区域在通道维度上拼接，通道数变为原来的 4 倍。
        卷积降维：通过 
        1×1
         卷积将通道数从 4*c1 降为目标通道数 c2，同时融合特征。
        输入输出形状
        输入：(batch_size, channels, height, width)
        输出：(batch_size, channels*4, height/2, width/2)
        
        核心优势
            无需池化：通过切片和拼接替代传统池化，避免信息损失。
            特征浓缩：将空间信息编码到通道中，增强特征表达能力。
            计算高效：相比直接下采样，能保留更多细节信息。
            应用场景
            YOLOv5：作为骨干网络的初始层，快速降低分辨率并增加通道数。
            轻量级网络：替代池化层，在资源受限场景中平衡精度和效率。
...
class GSConv(nn.Module):
    ...
    reshape操作：
        ...
        将输入张量 x2 从形状 [B, C, H, W] 变为 [B, 2, C/2, H, W]
        这相当于将通道分成2组，每组包含C/2个通道
        permute操作：
        
        交换维度1和2，从 [B, 2, C/2, H, W] 变为 [B, C/2, 2, H, W]
        这相当于将两组通道交错排列
        最终reshape：
        
        将形状恢复为 [B, C, H, W]，但此时通道顺序已被重新排列
    """
