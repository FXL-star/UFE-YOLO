...
__all__ = ['MultipoleAttention','C2f_MultipoleAttention','C2f_MultipoleBlock',
           'PSA_MultipoleAttention','PSA_MultipoleAttention_DyT']
...
class MultipoleAttention(nn.Module):
    """
    B C H W Multipole Attention 多极注意力
    """
...
class Bottleneck_MultipoleAttention(nn.Module):
    """定义Bottleneck_MultipoleAttention，替换C2f中Bottleneck位置"""
...
class C2f_MultipoleAttention(nn.Module):
    """C2f_MultipoleAttention，通过n配置Bottleneck_MultipoleAttention个数"""
    ...
    def __init__(self, c1, c2, n=1,HW=20, shortcut=False, g=1, e=0.5):
        ...
        # 替换m为Bottleneck_MultipoleAttention
...
class DynamicTanh(nn.Module):
    ...
    def forward(self, x):
        ...
        return x
class PSA_MultipoleAttention(nn.Module):

    def __init__(self, c1, c2, HW=20,e=0.5):
        super().__init__()
        assert (c1 == c2)
        self.c = int(c1 * e)
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        ...
        self.attn = MultipoleAttention(self.c, HW)
...
class PSA_MultipoleAttention_DyT(nn.Module):
    def __init__(self, c1, c2, HW=20, e=0.5):
        super().__init__()
        assert (c1 == c2)
        self.c = int(c1 * e)
        ...
        self.dyt1 = DynamicTanh(normalized_shape=self.c)
        self.dyt2 = DynamicTanh(normalized_shape=self.c)

    def forward(self, x):
        a, b = self.cv1(x).split((self.c, self.c), dim=1)
        b = b + self.attn(self.dyt1(b))
        b = b + self.ffn(self.dyt1(b))
        return self.cv2(torch.cat((a, b), 1))
...
if __name__ == '__main__':
    ...
    MABlock = C2f_MultipoleBlock(32,32,1,64)  #第二个模块 ：多极注意力块
