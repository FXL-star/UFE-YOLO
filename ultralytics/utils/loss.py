...
class BboxLossShape_NWD(nn.Module):
    """Criterion class for computing training losses during training."""
...
class BboxLossE(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        ...
        if self.nwd_loss:
            nwd = wassersteinLoss(pred_bboxes[fg_mask], target_bboxes[fg_mask])
            nwd_loss = ((1.0 - nwd) * weight).sum() / target_scores_sum
...
# Slide Loss，解决简单样本和困难样本之间的不平衡问题
...
# 自适应阈值焦点损失（ATFL） 是一种用于解决 红外小目标检测中目标与背景不平衡问题 的创新方法。以下将从其 设计出发点 、 原理 、 结构 和 优势 进行详细介绍，并结合文中相关图示辅助说明。
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk=10):  # model must be de-paralleled
        ...
        self.bce = nn.BCEWithLogitsLoss(reduction="none")  # 交叉熵损失
        # self.bce = SlideLoss(nn.BCEWithLogitsLoss(reduction="none"))  # 采用Slide Loss，解决简单样本和困难样本之间的不平衡问题

        g = 1  # focal loss gamma 使用AdaptiveThresholdFocalLoss
        if g > 0:
            self.bce = AdaptiveThresholdFocalLoss(self.bce, g)

        self.hyp = h  # hyperparameters 超参数
        ...
        # TODO 这个的作用：这个是一个assign算法，用于计算每个预测框与真实框之间的匹配关系，并返回匹配结果。
        self.assigner = TaskAlignedAssigner(topk=tal_topk, num_classes=self.nc, alpha=0.5, beta=6.0)
        # self.bbox_loss = BboxLoss(m.reg_max).to(device)
        self.bbox_loss = BboxLossShape_NWD(m.reg_max).to(device)
        # self.bbox_loss = BboxLossE(m.reg_max, use_dfl=self.use_dfl, nwd_loss=self.nwdloss, iou_ratio=self.iou_ratio).to(device)
...
def wassersteinLoss(pred, target, eps=1e-7, constant=12.8):
